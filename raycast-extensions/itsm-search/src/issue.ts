...
const fields = "summary,issuetype,status,reporter";
...
export async function searchIssues(query: string): Promise<ResultItem[]> {
  ...
  const mapResult = async (issue: Issue): Promise<ResultItem> => ({
    ...
    subtitle: `${issue.key} · ${issue.fields.issuetype.name}· ${issue.fields.reporter.displayName}`,
...
export default function SearchIssueCommand() {
  return SearchCommand(searchIssues, "通过事件 主题, @项目, #事件类型, !状态 and %经办人 来检索事件");
}
