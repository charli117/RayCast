...
const defaultStatusErrors: StatusErrors = {
  401: ErrorText("ITSM 鉴权失败", "请登录 https://itsm.cvte.com/secure/ViewProfile.jspa 重新获取个人访问令牌并配置到插件中."),
};
...
function throwIfResponseNotOkay(response: Response, statusErrors?: StatusErrors) {
  if (!response.ok) {
    ...
    else if (status >= 500) throw new PresentableError("ITSM Error", `Server error ${status}`);
    else throw new PresentableError("ITSM Error", `Request error ${status}`);
  }
}
